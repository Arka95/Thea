"""
motion_assessment/optical_flow.py — Optical flow computation with GPU/CPU fallback.

Provides a factory function returning a reusable flow calculator.
The calculator is created ONCE and reused across all frames for efficiency.
"""
...
def create_flow_calculator(config: dict) -> FlowCalculator:
    ...
    if gpu_enabled and gpu_available:
        ...
        if gpu_enabled and not gpu_available:
            logger.warning("GPU requested but no CUDA device found. Falling back to CPU.")
