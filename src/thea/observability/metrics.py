"""
thea.observability.metrics — Execution metrics collection and reporting.

Provides decorator-based instrumentation for pipeline functions with:
...
Design:
  ...
  - Decorator is opt-in on major functions only

Usage:
    from thea.observability import track, MetricsCollector
...
logger = logging.getLogger("thea.observability.metrics")
...
class MetricsCollector:
        ...
        Returns:
            {
                ...
                "success_count": int,
                "error_count": int,
                "functions": { name: {calls, total_sec, avg_ms, min_ms, max_ms, errors} }
            }
        """
        with self._lock:
    ...
    def report_table(self) -> str:
        ...
        lines.append("-" * 85)
        lines.append(
            f"Total: {s['total_functions']} calls, {s['total_duration_sec']:.3f}s, "
...
def track(collector: MetricsCollector = None, context_fn: Callable = None):
    ...
    Usage:
        ...
        @track(context_fn=lambda a, kw: {"video": a[0]})
